...
This part of the Data Visualisation Masterclass is intended to teach you some very basic techniques to visualise tabular data, i.e. data arranged in a table. We will refer to columns in the table as dimensions or variables, and to rows of the table as items or data points.
...
## Stacked bar chart
Stacked bar charts make no sense for stats that cannot be summed:
```{r}
...
Normalized stacked bar charts fill up to 100%:
```{r}
...
Note that each normalized stacked bar corresponds to a Cartesian version of a pie chart:
```{r}
...
## Histogram
The default histogram uses weird bins:
```{r}
...
Hence, adjusting not only the binwidth, but also the position can have a huge impact on the visualisation - and its interpretation:
...
The binwidth is a crucial parameter that changes the appearance of the distribution drastically:
```{r}
histogram_2 <- ggplot(data, aes(Acceleration)) + geom_histogram(breaks = c(seq(14,24,by=2))) + scale_x_continuous(breaks = c(14,16,18,20,24)) + xlim(14,24)
histogram_2
```
